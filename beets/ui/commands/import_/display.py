...
class ChangeRepresentation:
    ...
    @cached_property
    def indent(self) -> str:
        return self._indentation_config["match_header"].as_number()

    @cached_property
    def indent_header(self) -> str:
        return ui.indent(self.indent)
    ...
    def show_match_header(self) -> None:
        ...
        if penalties := self.match.distance.penalties:
            parts.append(
                ui.colorize("changed", f"\u2260 {', '.join(penalties)}")
            )
        ...
        ui.print_(textwrap.indent("\n".join(parts), ui.indent(self.indent)))
