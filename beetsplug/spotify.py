...
class SpotifyPlugin(SearchApiMetadataSourcePlugin[SearchResponse]):
    ...
    def _search_api(
        self, query_type: Literal["album"], query: str, filters: dict[str, str]
    ) -> Sequence[SearchResponseAlbums]: ...

    @overload
    def _search_api(
        self, query_type: Literal["track"], query: str, filters: dict[str, str]
    ...
    def _match_library_tracks(
        ...
        for item in items:
            ...
            response_data_tracks = self._search_api(
                query_type="track", query=query_string, filters={}
            )
